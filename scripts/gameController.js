...
function seedLevels(total = 12) {
  return Array.from({ length: total }, (_, i) => {
    ...
    while (questions.length < 10 && attempts < 200){
      const q = generateQuestion(i, questions.length);
      const key = `${q.visualAid.fractions[0].numerator}/${q.visualAid.fractions[0].denominator}-${q.text.es.includes('pizza')?'p':'b'}`;
      if (!used.has(key)){
        used.add(key);
        questions.push(q);
      }
...
function renderGame() {
  ...
  questionCard.innerHTML = `<div class="card-title">${qText}</div>
...
function renderSurvivalGame() {
  ...
  const questionCard = document.createElement('div');
  questionCard.className = 'card';
  const qText = typeof question.text === 'string' ? question.text : (question.text.es || question.text.en);
  questionCard.innerHTML = `<div class="card-title">${qText}</div>
    ${renderVisualAid(question.visualAid)}
    <div id="options"></div>
    <div id="feedback"></div>`;
...
function nextQuestionOrLevel() {
  ...
  if (currentQuestionIdx < level.questions.length - 1) {
    ...
    if (levels[currentLevelIdx + 1]) {
      ...
      localStorage.setItem('levels', JSON.stringify(levels.map(l => ({ name: l.name, unlocked: l.unlocked }))));
    }
    currentLevelIdx++;
